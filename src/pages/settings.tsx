...
/**
 * 애플리케이션 설정을 위한 페이지 컴포넌트입니다.
 */
export default function SettingsPage() {
    return (
        <div className="h-full w-full flex flex-col p-6 overflow-y-auto bg-background text-foreground">
            {/* 페이지 헤더 */}
            <div className="mb-6">
                <h1 className="text-2xl font-bold tracking-tight">설정</h1>
                <p className="text-muted-foreground">애플리케이션 전역 설정을 구성합니다.</p>
                <p className="text-muted-foreground"> *변경사항은 자동 저장됩니다.</p>
            </div>

            {/* 설정 탭 */}
            <Tabs defaultValue="general" className="w-full flex-1 flex flex-col">
                <TabsList className="w-full grid grid-cols-4 sticky top-0 z-10 bg-background/95 backdrop-blur-sm shadow-md">
                    <TabsTrigger value="general" className="data-[state=active]:bg-blue-600 data-[state=active]:text-white">일반</TabsTrigger>
                    <TabsTrigger value="view" className="data-[state=active]:bg-blue-600 data-[state=active]:text-white">뷰/그리드</TabsTrigger>
                    <TabsTrigger value="coating" className="data-[state=active]:bg-blue-600 data-[state=active]:text-white">코팅</TabsTrigger>
                    <TabsTrigger value="gcode" className="data-[state=active]:bg-blue-600 data-[state=active]:text-white">G-Code 스니펫</TabsTrigger>
                </TabsList>
                <div className="mt-6">
                    <TabsContent value="general">
                        {/* 각 탭의 콘텐츠를 Card 컴포넌트로 감싸 시각적 그룹화를 강화합니다. */}
                        <Card>
