...
export function PathLayer({ isPanning = false }: PathLayerProps) {
    ...
    // path 생성 클래스
    const masker = new MaskingManager(gcodeSettings);
    const calc = new PathCalculator(gcodeSettings, masker);
        ...
        const schedule = (shape: CustomShapeConfig, delay: number) => {
            ...
            const timer = window.setTimeout(async () => {
                try {
                    ...
                    const exists = paths .some((g) => g.id === groupId);
                    const base = {
                        id: groupId,
    ...
    }, [dispatch, shapes, paths, shapeSnapshot, groupIdOf, toSegments, gcodeSettings, calc]);
    ...
    const renderSegment = useCallback(
        (segment: PathSegment, pathGroup: PathGroup, isSelected: boolean) => {
            ...
            const coatingWidth = gcodeSettings?.coatingWidth ?? 2;
    ...
    const renderGroup = useCallback(
        ...
        [isPanning, pathEvents, renderSegment,]
    );
